...
// Владельцы — безлимитный доступ без ключа
const OWNER_IPS = [];
const OWNER_TOKENS = ['iskarmel@gmail.com', 'asisaiagency@gmail.com'];
...
app.post('/api/analyze', freeLimiter, async (req, res) => {
  ...
  const isOwner = OWNER_TOKENS.includes(ownerToken);

  let isPremium = isOwner;

  if (!isOwner && license_key) {
